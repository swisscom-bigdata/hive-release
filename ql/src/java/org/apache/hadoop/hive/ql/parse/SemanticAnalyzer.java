...
public class SemanticAnalyzer extends BaseSemanticAnalyzer {
  ...
  private void replaceViewReferenceWithDefinition(QB qb, Table tab,
    ...
    try {
      ...
      viewTree = ParseUtils.findRootNonNullToken(
          pd.parse(viewText, ctx, tab.getCompleteName()));
      if (!unparseTranslator.isEnabled() &&
...
//todo: this looks different than current 'master'!!!!!!!!
  ...
  private Operator genTablePlan(String alias, QB qb) throws SemanticException {
    ...
    if (top == null) {
      ...
      ((TableScanOperator) top).setInsideView(qb.isInsideView()
          || qb.getAliasInsideView().contains(alias.toLowerCase()));
